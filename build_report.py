...
function toggleTable(btnId,contentId){const btn=document.getElementById(btnId),content=document.getElementById(contentId);if(!btn||!content)return;btn.addEventListener('click',()=>{content.classList.toggle('hidden');btn.classList.toggle('collapsed');btn.classList.toggle('expanded');btn.textContent=(btn.classList.contains('expanded')?'▼ Hide':'▶ Show ')+btn.textContent.slice(2);});}
toggleTable('emo-toggle','emo-table-content');
toggleTable('ratings-toggle','ratings-table-content');
const classesToggle=document.getElementById('classes-toggle'),classesDiv=document.getElementById('classes');if(classesToggle)classesToggle.addEventListener('click',()=>{classesDiv.classList.toggle('hidden');classesToggle.classList.toggle('collapsed');classesToggle.classList.toggle('expanded');classesToggle.textContent=(classesToggle.classList.contains('expanded')?'▼ Hide':'▶ Show ')+classesToggle.textContent.slice(2);});
